...
The local practice draft at [`individual/ram/practice.v`](individual/ram/practice.v) uses `mem[waddr]` and `mem[raddr]`, but it does not currently declare `mem`. The published active [`individual/ram/sync_ram.v`](individual/ram/sync_ram.v) contains the required declaration:
